...
#define QFUNC(var) QUOTE(DFUNC(var))
...
#ifdef ISDEV
    #define EFUNC(var1,var2) (currentNamespace getVariable [QEFUNC(var1,var2), {if (time > 0) then {["Error function %1 dont exist or isNil in %2 L%3", QEFUNC(var1,var2), __FILE__, __LINE__] call BIS_fnc_error;}; DUMP(QEFUNC(var1,var2) + " Dont Exist")}])
#endif
    ...
    #define EFUNC(var1,var2) {\
        ...
        private _tempRet = _this call (currentNamespace getVariable [QEFUNC(var1,var2), {if (time > 0) then {["Error function %1 dont exist or isNil in %2 L%3", QEFUNC(var1,var2), __FILE__, __LINE__] call BIS_fnc_error;}; DUMP(QEFUNC(var1,var2) + " Dont Exist")}]);\
...
#ifdef ISDEV
    #define CFUNC(var) (currentNamespace getVariable [QCFUNC(var), {if (time > 0) then {["Error function %1 dont exist or isNil", QCFUNC(var)] call BIS_fnc_error;}; DUMP(QCFUNC(var) + " Dont Exist")}])
#else
