...
private _output = "";
{
    ...
    _output = format ["     path = ""%1"";", _path] + toString [10];
    _output = format ["     offset[] = {%1, %2, %3};", _pos select 0, _pos select 1, _pos select 2] + toString [10];
    _output = format ["     dirVector[] = {%1, %2, %3};", _dir select 0, _dir select 1, _dir select 2] + toString [10];
    _output = format ["     upVector[] = {%1, %2, %3};", _up select 0, _up select 1, _up select 2] + toString [10];
